...
//						`SimTask`.`AuthLibUserId`=" . intval(AuthLib::getUserId()) . "
		...
		{
			// sett simStarted hvis output-filen eksisterer
			if ( file_exists(dirname(__FILE__) . "/../public_html/resultater/" . $row['xmlId'] . ".csv" ) )
			{
				$row['simStarted'] = TRUE;
			}
			else
...
//		print_r($weatherStations);
		...
		$infoMsg = "Oppdatering/innlegging av vaerstasjoner gikk bra ";
		$infoMsg .= " (Antall vaerstasjoner: " . count($weatherStations) . ")";
//		$infoMsg = "(Nye: ";
